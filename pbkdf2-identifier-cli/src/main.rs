...
fn main() {
    ...
    let matches = App::new("PBKDF2 Identifier")
        .version("0.2")
        .author("zer0x64")
        .about("Helps you find the parameters used to generate a PBKDF2 hash to break the \
    ...
    if algorithm_name == "all" {
        ...
        let (algorithm, iterations) = pbkdf2_identifier::identify_all(password.as_bytes(), &hash, &salt, max);
        if iterations == 0 {
            println!("Not found!")
        } else {
            println!("Found!\nAlgorithm: {}\nIterations: {}", algorithm.name(), iterations);
        }
    } else {
